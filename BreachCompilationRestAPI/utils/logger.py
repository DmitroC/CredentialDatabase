...
class Logger:
    ...
    USAGE:
            Logger(name='BreachCompilationRestAPI)

    """
    __instance = None

    def __init__(self, name='BreachCompilationRestAPI', level='info', log_folder='/var/log', log_file_size=10000000, debug=False):
        ...
        if not debug:
            ...
            info_log_file_path = log_folder + '/BreachCompilationRestAPI/info.log'
            error_log_file_path = log_folder + '/BreachCompilationRestAPI/error.log'
        else:
    ...
    def __create_log_folder(self, log_folder):
        """creates log folder in '/var/log/BreachCompilationRestAPI'

        """
        try:

            if log_folder.endswith('/'):
                if not os.path.exists(log_folder + 'BreachCompilationRestAPI'):
                    os.mkdir(log_folder + 'BreachCompilationRestAPI')
            else:
                if not os.path.exists(log_folder + '/BreachCompilationRestAPI'):
                    os.mkdir(log_folder + '/BreachCompilationRestAPI')
